...
# Load environment variables (for non-LinkUp settings)load_dotenv()


def get_llm_client():
    """Initialize and return the LLM client"""    return LLM(
        model="ollama/deepseek-r1",
        base_url="http://localhost:11434"    )

# Define LinkUp Search Toolclass LinkUpSearchInput(BaseModel):
    """Input schema for LinkUp Search Tool."""    query: str = Field(description="The search query to perform")
...
class LinkUpSearchTool(BaseTool):
    name: str = "LinkUp Search"    description: str = "Search the web for information using LinkUp and return comprehensive results"    args_schema: Type[BaseModel] = LinkUpSearchInput

    def __init__(self):
        super().__init__()

    def _run(self, query: str, depth: str = "standard", output_type: str = "searchResults") -> str:
        """Execute LinkUp search and return results."""        try:
            # Initialize LinkUp client with API key from environment variables            linkup_client = LinkupClient(api_key=os.getenv("LINKUP_API_KEY"))

            # Perform search            search_response = linkup_client.search(
                query=query,
                depth=depth,
            ...
            return f"Error occurred while searching: {str(e)}"def create_research_crew(query: str):
    """Create and configure the research crew with all agents and tasks"""    # Initialize tools    linkup_search_tool = LinkUpSearchTool()

    # Get LLM client    client = get_llm_client()
    ...
    # Define the research analyst    research_analyst = Agent(
    ...
    # Define the technical writer    technical_writer = Agent(
...
def run_research(query: str):
    """Run the research process and return results"""    try:
